...
<script type="text/javascript">
	...
	function toAdd(){
		art.dialog.open('${path}/train/toAddPayoutCommon.do',
				{title:"添加",
				width:'600px',
				height:'350px',
				background:'white',
	...
	function toExportPayoutCommonReport(){
		art.dialog.open('${path}/trainReport/toExportPayoutCommonReport.do',
				{title:"导出公共支出",
				width:'500px',
				height:'200px',
				background:'white',
	...
	function toPrint(id){
		window.open("${path}/train/toPayoutCommonPrint.do?id="+id);
	}
</script>
</head>
<body>
<div class="form_rowelem"> 
	<form action="${path}/train/toPayoutCommonManager.do" method="post" id="searchform">
        <div id="firstDiv" >
            <div class="form_rowelem">
 	...
 	<form id="form1" action="${path }/train/toPayoutCommonManager.do" method="post">
        ...
        <tr class="brown" >
         ...
         <c:forEach items="${pageList.list}" var="map">
			<tr >
				...
				<td>
					<pe:permission code="${menuCodeMap.payoutCommon }">
		...
		<c:if test="${fn:length(pageList.list) !=0}"> 
			<div class="form_rowelem">
				...
				<div class="table_page">
					<pe:permission code="${menuCodeMap.payoutExport }">
						<span class="form_button_btn" style="foalt:left;margin-top:1px; margin-left:10px;">
							<input type="button" onclick="toExportPayoutCommonReport()" class="form_button_btn_in" value="导出"/>
						</span>
					</pe:permission>
					...
					<c:import url="/common/plugs/pagePost.jsp">
						<c:param name="actionName">${path }/train/toPayoutCommonManager.do?1=1&formMap[type]=${queryMap.type }&formMap[createUserName]=${queryMap.createUserName }&formMap[note]=${queryMap.note }&formMap[beginTime]=${queryMap.beginTime }&formMap[endTime]=${queryMap.endTime }</c:param>
					</c:import>
				</div>
			</div> 
		</c:if>
