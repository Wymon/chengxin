...
<body>
	<table class="information_form" width="620" border="1px" cellspacing="0" cellpadding="0" align="center" style="width:620px;">
		...
		<tr>
			<td>应交总费用</td>
			<td>${printMap.allincome }</td>
			<td>应交学费</td>
			<td>${printMap.payable }</td>
		</tr>
		<tr>
			<td>总欠费</td>
			<td>
				<c:if test="${printMap.pstatus==0 }">
					${printMap.allincome-printMap.allpaying-printMap.paying }
				</c:if>
				<c:if test="${printMap.pstatus!=0 }">
					${printMap.allincome-printMap.allpaying }
				</c:if>
				</td>
			<td>该次缴费</td>
			<td>${printMap.paying }</td>
		</tr>
		<tr>
			<td>开单人</td>
