...
<div class="form_rowelem"> 
 	<form id="form1" action="${path }/trainBack/updateIncomeCommon.do" method="post">
	    ...
	    <div class="form_rowelem"> 
	        <label class="form_label"><font color="red" >*</font>项目：</label>
                <select id="type" class="form_select"  name="formMap[type]" onchange="changeType(this)">
	                 <c:forEach items="${incomeCommonItemList }" var="item">
	                 	<option value="${item.name }" <c:if test="${item.name==incomeCommonMap.type }">selected='selected'</c:if>>${item.name }</option>
	                 </c:forEach>
           		</select>
	    </div> 
	     <div class="form_rowelem" id="otherItem" style="display: none"> 
	        <label class="form_label"><font color="red">*</font>其它项目：</label>
	        <input type="text" name="formMap[otherType]" id="otherType" value="${incomeCommonMap.otherType }" size="20" maxlength="10" class="maxlength_20 form_input_text" style="width: 200px;" />
	    </div>
	    <div class="form_rowelem"> 
	        <label class="form_label"><font color="red" >*</font>金额：</label>
	        <input type="text" name="formMap[income]" id="income" value="${incomeCommonMap.income }" size="20" maxlength="50" class="maxlength_50 form_input_text" style="width: 200px;" />
	    </div> 
	    <div class="form_rowelem"> 
	        <label class="form_label">备注：</label>
	        <textarea name="formMap[note]" id="note" cols="35" rows="5">${incomeCommonMap.note }</textarea>
	    </div>
	    <div class="form_rowelem"> 
	        <label class="form_label">修改原因：</label>
	        <textarea name="formMap[updateNote]" id="updateNote" cols="45" rows="8">${incomeCommonMap.updateNote }</textarea>
	    </div> 
	    <input type="hidden" name="formMap[updateUser]" value="${user.id }"/>
	    <input type="hidden" name="formMap[incomeId]" value="${incomeCommonMap.id }"/>
	</form>
